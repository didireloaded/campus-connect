...
import { Plus, X, PenLine, Ghost, CalendarPlus, Camera } from "lucide-react";
...
const actions = [
  ...
  { icon: Camera, label: "Add Story", color: "bg-campus-purple", key: "story" },
];
...
export const CreateFAB = () => {
  ...
  const handleAction = (key: string) => {
    setOpen(false);
    if (key === "story") {
      // Story creation - future implementation
      return;
    }
    setSheet(key);
  };
        ...
        {open && (
          <div className="fixed bottom-20 right-4 z-50 flex flex-col-reverse items-end gap-3 max-w-lg">
            {actions.map((action, i) => (
              ...
              >
                <span className="bg-card shadow-elevated rounded-full px-3 py-1.5 text-sm font-medium text-foreground">
                  {action.label}
                </span>
